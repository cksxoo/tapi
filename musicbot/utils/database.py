...
class Database:
    ...
    def set_loop(self, guild_id: int, loop: int) -> None:
        ...
        if db_loop is None:
            ...
            # modify loop
            cur.execute(
                f"UPDATE {self.loop_table} SET loop_set=%s WHERE guild_id=%s",
                (loop, guild),
            )
    ...
    def get_loop(self, guild_id: int) -> int | None:
        ...
        # read loop
        cur.execute(
            f"SELECT * FROM {self.loop_table} WHERE guild_id=%s", (str(guild_id))
        )
        loop = cur.fetchone()
        con.close()
    ...
    def set_shuffle(self, guild_id: int, shuffle: bool) -> None:
        ...
        if db_shuffle is None:
            # add shuffle
            cur.execute(
                f"INSERT INTO {self.shuffle_table} VALUES(%s, %s)", (guild, shuffle)
            )
        else:
            # modify shuffle
            cur.execute(
                f"UPDATE {self.shuffle_table} SET shuffle=%s WHERE guild_id=%s",
                (shuffle, guild),
            )
    ...
    def get_shuffle(self, guild_id: int) -> bool | None:
        ...
        # read shuffle
        cur.execute(
            f"SELECT * FROM {self.shuffle_table} WHERE guild_id=%s", (str(guild_id))
        )
