...
def make_config() -> None:
    ...
    owners = input(
        "Enter owners id, If there are multiple owners, separate them with a comma : "
    )
    ...
    debug_server = input(
        "Enter DEBUG_SERVER id, If there are multiple DEBUG_SERVER, separate them with a comma : "
    )
    ...
    file.write(
        f'''from musicbot.sample_config import Config
